...
- (id)initWithDict:(NSMutableDictionary *)inDict{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",kAPIBaseURL,SENDSMSMAIL];
...
- (void)requestFinished:(ASIHTTPRequest *)req{
  ...
  self.dictionary  = [parser objectWithString:responseString error:&err];
    ...
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(ConfirmOrderRequestDelegate)]))
    {
        if (![[self.dictionary valueForKey:CODE] isEqualToString:OK] ) {
            ...
            [self.requestDelegate performSelectorOnMainThread:@selector(ConfirmOrderRequestFailedWithErrorMessage:) withObject:ErrorMsg waitUntilDone:NO];
        }
        else {
            
            [self.requestDelegate performSelectorOnMainThread:@selector(ConfirmOrderRequestFinishedWithSuccessMessage:) withObject:self.dictionary waitUntilDone:NO];
        }
    }
...
- (void)requestFailed:(ASIHTTPRequest *)req{
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(ConfirmOrderRequestDelegate)]))
    {
        [self.requestDelegate performSelectorOnMainThread:@selector(ConfirmOrderRequestFailedWithErrorMessage:) withObject:@"Request cannot be completed" waitUntilDone:NO];
    }
}
@end
