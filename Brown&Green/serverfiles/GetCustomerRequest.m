...
- (id)initWithDict:(NSMutableDictionary *)inDict{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",kAPIBaseURL,GetCustomer];
...
- (void)requestFinished:(ASIHTTPRequest *)req{
    ...
    NSLog(@"Response_GetCustomerRequest= %@",responseString);
  ...
  self.dictionary  = [parser objectWithString:responseString error:&err];
    ...
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(GetCustomerRequestDelegate)]))
    {
        if (![[self.dictionary valueForKey:CODE] isEqualToString:OK] ) {
            ...
            [self.requestDelegate performSelectorOnMainThread:@selector(GetCustomerRequestFailedWithErrorMessage:) withObject:ErrorMsg waitUntilDone:NO];
        }
        else {
            
            [self.requestDelegate performSelectorOnMainThread:@selector(GetCustomerRequestFinishedWithSuccessMessage:) withObject:self.dictionary waitUntilDone:NO];
        }
    }
...
- (void)requestFailed:(ASIHTTPRequest *)req{
    ...
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(GetCustomerRequestDelegate)]))
    {
        [self.requestDelegate performSelectorOnMainThread:@selector(GetCustomerRequestFailedWithErrorMessage:) withObject:@"Request cannot be completed" waitUntilDone:NO];
    }
}
@end
