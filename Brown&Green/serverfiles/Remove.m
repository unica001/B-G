...
- (id)initWithDict:(NSMutableDictionary *)inDict{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",kAPIBaseURL,RemovePromoCode];
...
- (void)requestFinished:(ASIHTTPRequest *)req{
  ...
  self.dictionary  = [parser objectWithString:responseString error:&err];
    ...
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(RemovePromoCodeRequestDelegate)]))
    {
        if (![[self.dictionary valueForKey:CODE] isEqualToString:OK] ) {
            ...
            [self.requestDelegate performSelectorOnMainThread:@selector(RemovePromoCodeRequestFailedWithErrorMessage:) withObject:ErrorMsg waitUntilDone:NO];
        }
        else {
            
            [self.requestDelegate performSelectorOnMainThread:@selector(RemovePromoCodeRequestFinishedWithSuccessMessage:) withObject:self.dictionary waitUntilDone:NO];
        }
    }
...
- (void)requestFailed:(ASIHTTPRequest *)req{
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(RemovePromoCodeRequestDelegate)]))
    {
        [self.requestDelegate performSelectorOnMainThread:@selector(RemovePromoCodeRequestFailedWithErrorMessage:) withObject:@"Request cannot be completed" waitUntilDone:NO];
    }
}
@end
