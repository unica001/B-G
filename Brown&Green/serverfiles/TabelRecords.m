...
@implementation TabelRecords
- (id)initWithDict:(NSMutableDictionary *)inDict{
    
    NSString *url = [NSString stringWithFormat:@"%@%@",kAPIBaseURL,GETUPDATEDTABELRECORDS];
...
- (void)requestFinished:(ASIHTTPRequest *)req{
	...
	NSLog(@"Response_TabelRecords= %@",responseString);
	...
	if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(TabelRecordsDelegate)]))
	{
		if (![[self.dictionary valueForKey:CODE] isEqualToString:OK] ) {
			...
			[self.requestDelegate performSelectorOnMainThread:@selector(tabelRecordsRequestFailedWithErrorMessage:) withObject:ErrorMsg waitUntilDone:NO];
		}
		else {
            
			[self.requestDelegate performSelectorOnMainThread:@selector(tabelRecordsRequestFinishedWithSuccessMessage:) withObject:self.dictionary waitUntilDone:NO];
		}
	}
...
- (void)requestFailed:(ASIHTTPRequest *)req{
	...
	if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(TabelRecordsDelegate)]))
	{
		[self.requestDelegate performSelectorOnMainThread:@selector(tabelRecordsRequestFailedWithErrorMessage:) withObject:@"Request cannot be completed" waitUntilDone:NO];
	}
}
