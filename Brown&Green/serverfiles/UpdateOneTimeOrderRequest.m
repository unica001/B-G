...
@implementation UpdateOneTimeOrderRequest

- (id)initWithDict:(NSMutableDictionary *)inDict{
    
    NSString *url = [NSString stringWithFormat:@"%@%@",kAPIBaseURL,UPDATEONETIMEORDER];
...
- (void)requestFinished:(ASIHTTPRequest *)req{
    ...
    NSLog(@"Response_UpdateOneTimeOrderRequest= %@",responseString);
  ...
  self.dictionary  = [parser objectWithString:responseString error:&err];
    ...
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(UpdateOneTimeOrderRequestDelegate)]))
    {
        if (![[self.dictionary valueForKey:CODE] isEqualToString:OK] ) {
            ...
            [self.requestDelegate performSelectorOnMainThread:@selector(UpdateOneTimeOrderRequestFailedWithErrorMessage:) withObject:ErrorMsg waitUntilDone:NO];
        }
        else {
            
            [self.requestDelegate performSelectorOnMainThread:@selector(UpdateOneTimeOrderRequestFinishedWithSuccessMessage:) withObject:self.dictionary waitUntilDone:NO];
        }
    }
...
- (void)requestFailed:(ASIHTTPRequest *)req{
    ...
    if(self.requestDelegate != nil && ([self.requestDelegate conformsToProtocol:@protocol(UpdateOneTimeOrderRequestDelegate)]))
    {
        [self.requestDelegate performSelectorOnMainThread:@selector(UpdateOneTimeOrderRequestFailedWithErrorMessage:) withObject:@"Request cannot be completed" waitUntilDone:NO];
    }
}
@end
